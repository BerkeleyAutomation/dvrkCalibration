...
class Experiment:
    ...
    def __init__(self, config):
        ...
        self.config.input_dim = (
            self.training_histories.shape[2]
            if config.rnn
            else self.training_histories.shape[1] + self.training_phys.shape[1]
        )
        ...
        self.optimizer = Adam(
            list(self.forward_model.parameters()) + list(self.inverse_model.parameters()), lr=config.lr
        )
        self.rnn = config.rnn
        ...
        self.config.save_dir = self.save_dir = osp.join(
            self.save_dir, datetime.datetime.now().strftime("%Y-%m-%d--%H:%M:%S")
        )
        os.makedirs(self.save_dir)
        ...
        self.final_save_location = osp.join(
            self.save_dir, "../../../../../dvrk_2024/dvrk/calibration_models_psm" + str(psm_number)
        )
        if not os.path.exists(self.final_save_location):
            os.makedirs(self.final_save_location)
        with open(osp.join(self.final_save_location, "config.pkl"), "wb") as f:
            pickle.dump(self.config, f)
    ...
    def run(self):
        for i in range(self.training_iterations):
            ...
            fwd_loss = compute_standard_loss(
                self.forward_model, batch_histories, batch_cmds, batch_phys, True, self.rnn, self.device
            )
            inv_loss = compute_standard_loss(
                self.inverse_model, batch_histories, batch_cmds, batch_phys, False, self.rnn, self.device
            )
            consistency_loss = compute_cyclic_losses(
                self.forward_model, self.inverse_model, batch_histories, batch_cmds, batch_phys, self.rnn, self.device
            )
            ...
            with torch.no_grad():
                fwd_loss_val = compute_standard_loss(
                    self.forward_model, val_batch_histories, val_batch_cmds, val_batch_phys, True, self.rnn, self.device
                )
                inv_loss_val = compute_standard_loss(
                    self.inverse_model,
                    val_batch_histories,
                    val_batch_cmds,
                    val_batch_phys,
                ...
                consistency_loss_val = compute_cyclic_losses(
                    self.forward_model,
                    self.inverse_model,
                    val_batch_histories,
                    val_batch_cmds,
            ...
            if i % self.log_freq == 0:
                print(
                    "Iteration %d. Forward Training Loss: %8.5f Forward Validation Loss: %8.5f Inverse Training Loss %8.5f Inverse Validation Loss %8.5f Consistency Training Loss %8.5f Consistency Validation Loss %8.5f"
                    % (
                        i,
                        untorchify(fwd_loss),
                        untorchify(fwd_loss_val),
                        untorchify(inv_loss),
                        untorchify(inv_loss_val),
                        untorchify(consistency_loss),
                        untorchify(consistency_loss_val),
                    )
                )
    ...
    def save(self, fname):
        ...
        if fname == "model":
            save_dir = self.final_save_location
        torch.save(self.forward_model.state_dict(), osp.join(save_dir, forward_name))
        torch.save(self.inverse_model.state_dict(), osp.join(save_dir, inverse_name))
        print("Saved in " + str(save_dir))
    ...
    def plot(self, save=False, show=True):
        ...
        # Plot Forward Losses
        plt.plot(
            np.ones_like(self.logs.forward.training_losses) * original_loss,
            c="g",
            label="Original Loss (No correction)",
        )
...
def create_config():
    ...
    config.save_dir = save_dir  # where to save logs and models
    config.save_freq = 10  # how often to save model
    config.log_freq = 10  # how often to print progress
    config.validation_prob = 0.1
    config.rnn = True
    config.consistency_weight = 1.0  # set to 0 to not use cyclic consistency loss
    return config
