...
class CalibratedMovement():
    def __init__(self, horizon, model_dir):
      ...
      self.sampling_level = 3

    def move(self, desired):
      self.add_to_cache(desired)
      q_cmd = self.get_next_cmd(self.use_sampling)
      self.movement_cache[-1] = q_cmd
      self.move_arm(q_cmd)
      return q_cmd

    def get_next_cmd(self, use_sampling):
    ...
    def get_delta_cmd(self):
        ...
        for i in range(self.num_delta_iters):
            q_cmd = self.get_corrected_cmd(q_cmd)
        return q_cmd
        # q_des + e_h = q_act
        # (q_des - e_h) + e_h = q_act
        # q_des = q_act (!!)
    ...
    def get_corrected_cmd(self, q_cmd):
        zeros = np.zeros(3)
        e_h = np.array(self.invoke_model(q_cmd))
        e_h = np.concatenate((zeros, e_h))
        return q_cmd - e_h
...
# use sampling
    ...
    def verification(self):
        try:
            root = '/home/hwangmh/pycharmprojects/FLSpegtransfer/'
            file_path = root + 'experiment/4_verification/trajectory/random/'
            traj = np.load(file_path + 'joint_des.npy')
            ...
            new_q_act = []
            new_q_des = []
            for qd1,qd2,qd3,qd4,qd5,qd6 in traj[25:125]:
                ...
                print('index: ', len(new_q_des),'/',len(traj[25:125]))
...
model_dir = "./model/"
CB = CalibratedMovement(6, model_dir)
CB.verification()