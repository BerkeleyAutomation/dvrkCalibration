...
class dvrkCalibration:
    def __init__(self, know_transform):
        ...
        self.joint_trajectory_filename_ = os.path.join(
            self.calibration_output_path, "prime_psm" + self.psm_number + "_joint_trajectory.npy"
        )
    ...
    def collect_data_joint(self, j1, j2, j3, j4, j5, j6, transform="known"):  # j1, ..., j6: joint trajectory
        ...
        for qd1, qd2, qd3, qd4, qd5, qd6 in zip(j1, j2, j3, j4, j5, j6):
            ...
            # cv2.waitKey(0)
        np.save(self.joint_trajectory_filename_, np.array(q_des))
        # finally:
        #     import pdb
