...
class BlockDetection():
    def __init__(self):
        ...
        self.mask_dy = 70
        self.__theta = np.r_[-60:60:1]
        self.__number_pegs = 12
        # self.__masking_depth_block = [872, 884]
        # self.__masking_depth_peg = [855, 870]
        self.__masking_depth_block = [840, 860]
        self.__masking_depth_peg = [800, 840]

        # load mask
        filename = '/home/hwangmh/pycharmprojects/FLSpegtransfer/img/block_mask_filled.png'
        self.__mask = self.load_mask(filename, self.mask_dx, self.mask_dy, scaling=0.86)
    ...
    def locate_block(self, img, mask=None, downsample_factor=1):
        if len(img.shape) == 3:
            img = img[:, :, 0]
    ...
    def find_blocks2(self, blocks_masked, peg_points):
        ...
        # Segment block around each peg
        block_segmented = np.array([blocks_masked[c[1] - self.mask_dy//2:c[1]+self.mask_dy//2, c[0]-self.mask_dx//2:c[0] + self.mask_dx // 2] for c in peg_points])
        for n, b in enumerate(block_segmented):
            cnts, _ = cv2.findContours(b, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE )
            cnts = sorted(cnts, key=cv2.contourArea, reverse=True)
            if len(cnts)>1 and cv2.contourArea(cnts[0]) > 500:
                # get center of triangle
                mmt = cv2.moments(cnts[0])
                cx = int(mmt['m10'] / mmt['m00'])
                cy = int(mmt['m01'] / mmt['m00'])
                tx = cx - self.__mask.shape[1]//2
                ty = cy - self.__mask.shape[0]//2

                # get clean and filled block image
                block = np.zeros_like(b)
                cv2.drawContours(block, cnts, 0, (255,255,255), -1)

                mask_rotated = [self.transform_img(self.__mask, ang, tx, ty) for ang in self.__theta]
                n_cross = np.zeros_like(self.__theta)
                for i,mask in enumerate(mask_rotated):
                    block_crossed = cv2.bitwise_and(block, block, mask=mask)
                    n_cross[i] = np.shape(np.argwhere(block_crossed == 255))[0]

                angle = self.__theta[np.argmax(n_cross)]
                result_blocks.append([n+1, angle, tx, ty])  # n is the numbering of blocks
            else:
    ...
    def FLSPerception(self, img_depth):
        ...
        result_global_blocks, result_global_wo_blocks = self.find_blocks2(blocks_masked, peg_points)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if img_color == [] or img_depth == []:
            ...
            cv2.imshow("pegs_overlayed", pegs_overlayed)
            cv2.imshow("blocks_overlayed", blocks_overlayed)
            cv2.waitKey(1)