...
class BlockDetection():
    def __init__(self):
        ...
        self.__masking_depth_block = [874, 884]
        self.__masking_depth_peg = [855, 872]

        # load mask
        filename = 'img/block_sample_drawing.png'
        self.__mask = self.load_mask(filename, self.mask_dx, self.mask_dy, scaling=0.88)
    ...
    def find_blocks(self, blocks_masked, peg_points):
        try:
            ...
            block = np.array(
                [blocks_masked[c[1] - self.mask_dy//2:c[1]+self.mask_dy//2,c[0]-self.mask_dx//2:c[0]+self.mask_dx//2]
                 for c in peg_points])
            ...
            result_global_blocks = [[res[0], res[1], peg_points[res[0]-1][0]+res[2]-self.__mask.shape[0]//2,
                                     peg_points[res[0]-1][1]+res[3]-self.__mask.shape[1]//2] for res in result_blocks]
            result_global_wo_blocks = [[res[0], res[1], peg_points[res[0]-1][0]+res[2]-self.__mask.shape[0]//2,
                                        peg_points[res[0]-1][1]+res[3]-self.__mask.shape[1]//2] for res in result_wo_blocks]
        except:
            pass
    ...
    def FLSPerception(self, img_depth):
        ...
        blocks_overlayed = self.overlay_grasping_pose(blocks_overlayed, final_pp_larm, final_pp_rarm)
