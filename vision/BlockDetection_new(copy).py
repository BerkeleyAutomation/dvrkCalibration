...
class BlockDetection():
    def __init__(self):
        ...
        # self.__masking_depth_block = [0.862, 0.873]
        # self.__masking_depth_peg = [0.843, 0.861]
        # self.__masking_depth_block = [0.862, 0.878]
        # self.__masking_depth_peg = [0.843, 0.861]
        self.__masking_depth_block = [0.874, 0.884]
        self.__masking_depth_peg = [0.855, 0.872]

        # load mask
        filename = 'img/block_sample_drawing.png'
        # self.__mask = self.load_mask(filename, self.mask_dx, self.mask_dy, scaling=0.92)
        self.__mask = self.load_mask(filename, self.mask_dx, self.mask_dy, scaling=0.90)
        ...
        self.__sample_grasping_points = self.load_grasping_point(gp_number=5, dist_center=25, dist_gp=10)
        self.__sample_placing_points = self.load_grasping_point(gp_number=5, dist_center=18, dist_gp=10)
    ...
    def load_mask(self, filename, size_x, size_y, scaling=1.0):
        # load mask and scaling
        mask_inv = cv2.imread(filename, cv2.IMREAD_GRAYSCALE)
        ret, mask_inv = cv2.threshold(mask_inv, 100, 255, cv2.THRESH_BINARY)
        mask = cv2.bitwise_not(mask_inv)
        h = int(mask.shape[0]*scaling)
        w = int(mask.shape[1]*scaling)
        mask_scaled = cv2.resize(mask, dsize=(h, w))

        # fit to the given size
        dh = abs(size_y - h)
        dw = abs(size_x - w)
        if h > size_y:
            mask_sample = mask_scaled[dw/2:size_y, dh/2:size_x]
        else:
        ...
        try:
            ...
            block = np.array(
                [blocks_masked[c[1] - self.mask_dy / 2:c[1] + self.mask_dy / 2, c[0] - self.mask_dx / 2:c[0] + self.mask_dx / 2] for c
                 in peg_points])
            n_criteria = 150    # number of points that overlapped with the sample mask
            ...
            for n, b in enumerate(block):
                ...
                if n_max > n_criteria:  # if there is a block,
                    ...
                    if (n+1==1) or (n+1==2) or (n+1==3) or (n+1==7) or (n+1==8) or (n+1==10):
    ...
    def classify_blocks(self, result_global_blocks, result_global_wo_blocks):
        try:
            pair_larm = [[1,8], [3,7], [5,10], [8,1], [7,3], [10, 5]]
            pair_rarm = [[2,9], [4,12], [6,11], [9,2], [12, 4], [11,6]]
            pickup_blocks_larm = []
            place_blocks_larm = []
            pickup_blocks_rarm = []
            place_blocks_rarm = []
            for pl in pair_larm:
                ...
                if len(arg_pick)==0 or len(arg_place)==0:  # empty
                    ...
                    pickup_blocks_larm.append(result_global_blocks[arg_pick])
                    # result_global_wo_blocks[arg_place][1] = result_global_blocks[arg_pick][1]
                    place_blocks_larm.append(result_global_wo_blocks[arg_place])
            ...
            pass
        return pickup_blocks_larm, place_blocks_larm, pickup_blocks_rarm, place_blocks_rarm
    ...
    def get_sample_grasping_pose(self, angle_rotated, x, y, which_sample='pick'):
        ...
        if theta > 0:
            grasping_angle_rotated = [-30 + theta, -30 + theta, -30 + theta, -30 + theta, -30 + theta,
                                      -90 + theta, -90 + theta, -90 + theta, -90 + theta, -90 + theta,
                                      30 + theta, 30 + theta, 30 + theta, 30 + theta, 30 + theta]
        else:
            grasping_angle_rotated = [-30 + theta, -30 + theta, -30 + theta, -30 + theta, -30 + theta,
                                      90 + theta, 90 + theta, 90 + theta, 90 + theta, 90 + theta,
                                      30 + theta, 30 + theta, 30 + theta, 30 + theta, 30 + theta]
    ...
    def find_grasping_pose(self, result_pickup_block, peg_points, which_side):
        ...
        for res in result_pickup_block:
            n, theta, x, y = res
            gp_rotated_global = np.array(self.get_sample_grasping_pose(theta, x, y, 'pick')).reshape(5,3,4)
    ...
    def FLSPerception(self, img_depth):
        ...
        pickup_blocks_larm, place_blocks_larm, pickup_blocks_rarm, place_blocks_rarm = self.classify_blocks(result_global_blocks, result_global_wo_blocks)

        # Find grasping pose
        all_grasping_pose = self.find_all_grasping_pose(result_global_blocks)
        final_gp_larm = self.find_grasping_pose(pickup_blocks_larm, peg_points, 'left')
        final_gp_rarm = self.find_grasping_pose(pickup_blocks_rarm, peg_points, 'right')
        final_pp_larm = self.find_placing_pose(final_gp_larm, place_blocks_larm, peg_points)
        ...
        blocks_overlayed = self.overlay_block_contour(blocks_overlayed, pickup_blocks_larm)
        blocks_overlayed = self.overlay_block_contour(blocks_overlayed, pickup_blocks_rarm)
        blocks_overlayed = self.overlay_block_contour(blocks_overlayed, place_blocks_larm)
        blocks_overlayed = self.overlay_block_contour(blocks_overlayed, place_blocks_rarm)
        blocks_overlayed = self.overlay_grasping_pose(blocks_overlayed, final_gp_larm, final_gp_rarm)
        blocks_overlayed = self.overlay_grasping_pose(blocks_overlayed, all_grasping_pose, all_grasping_pose)
        # blocks_overlayed = self.overlay_numbering(blocks_overlayed, all_grasping_pose)

        return final_gp_larm, final_pp_larm, final_gp_rarm, final_pp_rarm, peg_points, pegs_overlayed, blocks_overlayed
