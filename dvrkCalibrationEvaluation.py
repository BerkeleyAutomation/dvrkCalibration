...
def single_psm_warmup(psm_number, psm):
    ...
    if psm_number == "1":
        psm_warmup_points = [
            np.array([[0.01], [0.0], [-0.16257132], [1]]),
            np.array([[-0.00584999], [0.02838133], [-0.15667597], [1]]),
            np.array([[0.05776214], [0.02180665], [-0.12783207], [1]]),
            np.array([[0.02882365], [0.05078037], [-0.1455797], [1]]),
        ]
        psm_warmup_quats = [
            np.array([-0.10979732, 0.04588188, -0.01995157, 0.99269398]),
            np.array([-0.02595783, 0.41877539, 0.59547544, 0.68510026]),
            np.array([0.30067317, -0.53993203, -0.38256866, 0.68681167]),
            np.array([0.00757072, 0.12789299, -0.33359552, 0.93397007]),
        ]
    elif psm_number == "2":
    ...
    for p1, q1 in zip(psm_warmup_points, psm_warmup_quats):
        ...
        time.sleep(1)
    psm_pose = np.array([0, 0, -0.135]), np.array([0, 0, 0, 1])
...
psm = dvrkArm("/PSM" + psm_number_input, dvrk_type=dvrk_type, use_rnn=False)
trajectory_type = input("Random Trajectory (1) or Suturing Trajectory (2)")
if trajectory_type == "1":
    eval_traj_path = os.path.join(
        root_path,
        "experiment/0_trajectory_extraction/shallow_and_deep_calibration_outputs/prime_psm"
        + psm_number_input
        + "_random_joint_eval_in_distribution.npy",
    )
    if not os.path.exists(eval_traj_path):
        import pdb

        pdb.set_trace()
        random_traj_path = os.path.join(
            root_path,
            "experiment/0_trajectory_extraction/shallow_and_deep_calibration_outputs/prime_psm"
            + psm_number_input
            + "_random_sampled.npy",
        )
        random_joint_traj = np.load(random_traj_path)
        sample_indices = np.random.choice(random_joint_traj.shape[0], size=20, replace=False)

        # Extract the sampled rows
        sampled_traj = random_joint_traj[sample_indices]

        np.save(eval_traj_path, sampled_traj)
    else:
        sampled_traj = np.load(eval_traj_path)
    pose_lists = [
    ...
    if psm_number_input == "1":
        ...
        pose_lists.append(
            (
                np.array([0.1088187, 0.08370875, -0.14635209]),
        ...
        pose_lists.append(
            (
                np.array([0.12961172, 0.08416236, -0.14685837]),
                np.array([0.61499985, 0.34896301, -0.34896301, 0.61499985]),
            )
        )
        pose_lists.append(
            (
                np.array([0.12417177, 0.08416236, -0.14876189]),
        ...
        pose_lists.append(
            (
                np.array([0.11521647, 0.08416236, -0.13590351]),
        ...
        pose_lists.append(
            (
                np.array([0.11024665, 0.08416236, -0.14622345]),
