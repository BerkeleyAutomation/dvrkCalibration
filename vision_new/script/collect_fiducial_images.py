...
for i in range(sampling):
    ...
    img_color, img_depth, img_point = zc.capture_3Dimage(img_crop=False, color='BGR')

    os.makedirs("fiducial_images/" + str(i), exist_ok=True)
    np.save("fiducial_images/" + str(i) + "/color", img_color)
    np.save("fiducial_images/" + str(i) + "/depth", img_depth)
    np.save("fiducial_images/" + str(i) + "/point", img_point)
    color = cv2.cvtColor(img_color, cv2.COLOR_RGB2BGR)
    cv2.imwrite("fiducial_images/" + str(i) + "/color.png", color)